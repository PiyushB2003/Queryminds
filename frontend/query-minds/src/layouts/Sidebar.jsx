...
const Sidebar = () => {
    const { setRecentPrompt, setShowResult, count, setCount, expanded, setExpanded, handleClick, isPending, error, data, } = useContext(Context);
    ...
    return (
        <div className={` ${expanded ? "w-20 sm:flex hidden transition-all ease-in-out duration-300 bg-[#0c1649] flex-col text-white justify-between" : " transition-all hidden left-0 ease-in-out duration-300l w-80 bg-[#0c1649] text-white sm:flex flex-col h-screen"} `}>
            <div className={`${expanded ? "w-full border-none flex flex-col justify-center py-4" : "py-4 font-semibold border-b border-gray-700"}`}>
                <div className={`${expanded ? "flex flex-col w-full justify-center items-center" : "flex justify-between"}`}>
                    <span className={`${expanded ? "mx-0 p-3 cursor-pointer hover:bg-gray-700 md:flex items-center justify-center rounded-full" : "cursor-pointer hover:bg-gray-700 mx-4 p-2 md:flex items-center justify-center rounded-full"}`} onClick={() => setExpanded(curr => !curr)}>
